...
// 1. Replace VCInputBox completely using string indexing
const vcStart = code.indexOf('function VCInputBox({ form, onFormChange, onSubmit, loading, fileInputRef, expanded, onExpand }: VCInputBoxProps) {');
...
`;
  ...
  console.log("VCInputBox not found.");
}

// 2. Replace Canvas Mode input with agent-search-pill
const canvasStart = code.indexOf('<form className="vc-canvas-input-form"');
const canvasEnd = code.indexOf('</form>', canvasStart);
...
`;
  code = code.substring(0, canvasStart) + newCanvasInput + code.substring(canvasEnd);
  console.log("Canvas input replaced.");
} else {
  console.log("Canvas input not found.");
}
