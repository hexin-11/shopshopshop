...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('onBack={() => setVcStage("canvas")}')) {
    startIdx = i - 10;
    // Walk forward until we find `<section className={\`agent-home`
    for (let j = i; j < lines.length; j++) {
      if (lines[j].includes('agent-home-empty')) {
        endIdx = j - 3;
        break;
      }
    }
...
if (startIdx !== -1 && endIdx !== -1) {
  const insert = `                <StoryboardAdjustView
                  project={vcProject}
  ...
  lines.splice(startIdx, endIdx - startIdx + 1, ...insert.split('\n'));
  fs.writeFileSync(path, lines.join('\n'));
  console.log('Fixed EXACTLY! Deleted from', startIdx, 'to', endIdx);
} else {
