...
// 1. Replace handleStartGeneration and handleRegenerateBeat
const newHandlers = `
...
txt = txt.replace(
  /const handleStartGeneration = async \(\) => \{[\s\S]*?const submitCanvasInput = async \(\) => \{/,
  newHandlers + '\n  const submitCanvasInput = async () => {'
);

// 2. Replace image rendering with video
txt = txt.replace(
